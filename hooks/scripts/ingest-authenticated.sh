#!/bin/bash
# Engram ingestion hook with OAuth authentication
# For cloud deployments where ingestion requires bearer token auth.
...
# Get ingestion URL from environment or use default cloud URL
...
if [ -n "$AUTH_HEADER" ]; then
	...
	# Fall back to unauthenticated request (will fail in cloud, but try anyway)
	(curl -sS -X POST "${INGESTION_URL}/v1/ingest" \
		-H "Content-Type: application/json" \
		-d "$PAYLOAD" \
		--max-time 5 \
